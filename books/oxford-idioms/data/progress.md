...
- **`pdf/牛津习语词典.pdf`** ← 打印版，273 页 / 5 MB，16 开 185×260mm 竖版，A-Z 书签 24 条
...
## PDF 版式
16 开 185×260mm 竖版，四列宽度 21% / 17% / 7% / 55%。渲染器和行数约束沿用 GRE3000
踩出来的经验（见那本的 progress）：必须用 playwright 自带 Chromium，量行数的页面
必须 `viewport=609px` + `emulate_media("print")`，行数必须用 Range 行盒实测。
- **习语列和例句列允许三行，中间两列压两行**。四列都卡两行的话例句会被缩到 4pt 上下，
  完全没法看。生成时会打印「实测仍超过两行的格子」，当前是 0。
