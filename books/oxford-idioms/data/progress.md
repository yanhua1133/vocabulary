...
- **绝不出现第三行**。四列一律压两行，靠三件事做到：
  ...
  ② **字号下限按列分开定**：习语列 2.9pt，中文两列 4.6pt，例句列 4.4pt。
     习语列那几十条超长条目只能靠小字，好在都是英文，比中文耐看；
     中文绝不能低于 4.6pt。
  ③ 原书例句太长的（半角宽 > 170）直接弃用、退回自拟的那句短例句。
  成品复核：2.9 万个格子里 1 行 6687、2 行 22082，**三行 0 个**；
  行高中位 21.7pt、九成 22.6pt、最大 22.6pt，基本齐平。
